...
void FlowerGen::_bind_methods() {
    ...
    ADD_PROPERTY(PropertyInfo(Variant::REAL, "petal_width", PROPERTY_HINT_RANGE, "0.01,10,0.01"), "set_petal_width", "get_petal_width");
    ADD_PROPERTY(PropertyInfo(Variant::REAL, "petal_height", PROPERTY_HINT_RANGE, "0.01,10,0.01"), "set_petal_height", "get_petal_height");
    ...
    ADD_PROPERTY(PropertyInfo(Variant::REAL, "petal_scale", PROPERTY_HINT_RANGE, "0.01,10,0.01"), "set_petal_scale", "get_petal_scale");
...
Transform IDENTITY1 = Transform(Basis(1, 0, 0, 0, 1, 0, 0, 0, 1 ),Vector3(0,0,0));
...
void FlowerGen::create_flower(Array &p_array, Array transforms) const{
    ...
    for(int i = 0; i < petals; i++) {
        ...
        for(int j = 0; j < petal_rings; j++) {
            ...
            float petalangle = Math::lerp_angle(-petal_angle.x, -petal_angle.y,((float)j + 1.0 + (float)i/float(petals) )/ ((float)petal_rings));
            Transform petalTransform = IDENTITY1;
            ...
            petalTransform = (IDENTITY1* transforms[leaft])*petalTransform  ;
    ...
    p_array[VS::ARRAY_VERTEX] = points;
	p_array[VS::ARRAY_NORMAL] = normals;
	p_array[VS::ARRAY_TANGENT] = tangents;
	p_array[VS::ARRAY_TEX_UV] = uvs;
	p_array[VS::ARRAY_INDEX] = indices;
